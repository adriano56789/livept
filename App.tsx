...
import { ToastType, ToastData, Streamer, User, Gift, StreamSummaryData, LiveSessionState, RankedUser, Conversation, Country, NotificationSettings, BeautySettings, FeedPhoto, StreamHistoryEntry, Visitor, PurchaseRecord, MusicTrack } from './types';
...
import PrivateInviteNotificationModal from './components/PrivateInviteNotificationModal';
import MusicDetailScreen from './components/MusicDetailScreen';
...
const AppContent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const handlePrivateInvite = (payload: { stream: Streamer }) => {
            if (payload.stream.hostId === currentUser?.id) return;
            addToast(ToastType.Info, `${payload.stream.name} te convidou para uma sala privada!`);
            setInviteNotification(payload);
        };

        webSocketManager.on('streamerLive', handleStreamerLive);
        webSocketManager.on('privateStreamInvite', handlePrivateInvite);

        return () => {
            webSocketManager.off('streamerLive', handleStreamerLive);
            webSocketManager.off('privateStreamInvite', handlePrivateInvite);
        };
    }, [addToast, notificationSettings, allUsers, updateUserEverywhere, currentUser]);
    ...
    const checkMicrophonePermission = async () => {
        ...
        try {
            ...
            if (status === 'denied') {
                ...
                else if (permissionAction === 'createPost') setIsCreatePostOpen(true);
                setPermissionAction(null);
            } else {
    ...
    const handleOpenCreatePost = () => {
        checkCameraPermission('createPost');
    };
    ...
    const handleUseSound = (music: MusicTrack) => {
        setViewingMusic(null); // Close detail screen if it was open
        setMusicForPost(music);
        setIsCreatePostOpen(true); // Open create post screen with music
    };
    ...
    const handlePostComplete = (updatedUser: User) => {
        ...
        setActiveScreen('video'); // Switch to video screen to see the new post
    };
    ...
    return (
        <div className="h-full w-full bg-black text-white overflow-hidden relative font-sans" style={{
            paddingTop: 'var(--sat)',
            ...
            {activeStream && streamRoomData && currentUser ? (
                ...
                <>
                    <div className="h-full w-full">
                        ...
                        {activeScreen === 'video' && <VideoScreen currentUser={currentUser} onViewProfile={handleViewProfile} onPhotoLiked={() => setLastPhotoLikeUpdate(Date.now())} onViewMusic={(music) => setViewingMusic(music)} onFollowUser={handleFollowUser} lastPhotoLikeUpdate={lastPhotoLikeUpdate} onOpenPhotoViewer={(photos, index) => setPhotoViewerData({ photos, initialIndex: index })} onUseSound={handleUseSound} onOpenCreatePost={handleOpenCreatePost} />}
            ...
            {isGoLiveSetupOpen && <GoLiveScreen isOpen={isGoLiveSetupOpen} onClose={() => setIsGoLiveSetupOpen(false)} onStartStream={handleStartStream} addToast={addToast} currentUser={currentUser} />}
            ...
            {viewingMusic && <MusicDetailScreen music={viewingMusic} onClose={() => setViewingMusic(null)} onUseSound={handleUseSound} />}
            ...
            {inviteNotification && (
                <PrivateInviteNotificationModal
                    isOpen={!!inviteNotification}
                    onClose={() => setInviteNotification(null)}
                    onAccept={() => {
                        handleSelectStream(inviteNotification.stream);
                        setInviteNotification(null);
                    }}
                    inviterName={inviteNotification.stream.name}
                    inviterAvatar={inviteNotification.stream.avatar}
                />
            )}
