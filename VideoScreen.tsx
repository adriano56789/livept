...
import { HeartIcon, MessageIcon, ShareIcon, MusicNoteIcon, PlayIcon, CameraIcon, PlusIcon } from './components/icons';
...
const VideoSlide: React.FC<VideoSlideProps> = ({ video, onViewProfile, onPhotoLiked, isActive, onOpenComments, onViewMusic, onFollowUser, onUseSound }) => {
    ...
    useEffect(() => {
        ...
        if (isActive) {
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => {
                    if(audioRef.current) audioRef.current.play().catch(e => console.log("Audio play failed on active", e));
                    setIsPlaying(true);
                    setShowPlayIcon(false);
                }).catch(error => {
    ...
    const handleUseSoundClick = (e: React.MouseEvent) => {
        e.stopPropagation();
        if(!video.audioUrl && !video.musicTitle) return; // Cannot use sound from a video that has no audio and is not a remix
...
const VideoScreen: React.FC<VideoScreenProps> = ({ currentUser, onViewProfile, onPhotoLiked, onViewMusic, onFollowUser, lastPhotoLikeUpdate, onOpenPhotoViewer, onUseSound, onOpenCreatePost }) => {
    ...
    return (
        <div className="bg-black h-full text-white">
            <header className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between p-4">
                <h2 className="text-xl font-bold text-white [text-shadow:_0_1px_4px_rgb(0_0_0_/_50%)]">Vídeos</h2>
                <button onClick={onOpenCreatePost} className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center text-white backdrop-blur-sm shadow-lg">
