...
const MainScreen: React.FC<MainScreenProps> = ({ onOpenReminderModal, onOpenRegionModal, onSelectStream, onOpenSearch, streamers, isLoading, activeTab, onTabChange, showLocationBanner }) => {
  ...
  return (
    <div className="flex flex-col bg-[#111111] h-full pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
          ...
          <div className="flex space-x-6 overflow-x-auto whitespace-nowrap no-scrollbar">
            {tabs.map((tab) => (
              <button
                ...
                className={`py-3 text-sm font-medium transition-colors flex-shrink-0 ${activeTab === tab.key
